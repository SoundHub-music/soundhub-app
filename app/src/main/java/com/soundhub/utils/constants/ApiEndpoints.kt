...
sealed class ApiEndpoints {
	...
	data object LastFm {
		...
		const val GET_USER_INFO =
			"?$GET_USER_INFO_METHOD&$LAST_FM_API_KEY&$JSON_RESPONSE_FORMAT"

		const val GET_ARTIST_INFO =
			"?$GET_ARTIST_INFO_METHOD&$LAST_FM_API_KEY&$JSON_RESPONSE_FORMAT"

		const val GET_TOP_ARTISTS_BY_TAG_ENDPOINT = "?$GET_TOP_ARTISTS_METHOD&" +
				"$LAST_FM_API_KEY&" +
				JSON_RESPONSE_FORMAT

		const val SEARCH_ARTISTS_ENDPOINT =
			"?$SEARCH_ARTISTS_METHOD&$LAST_FM_API_KEY&" +
					JSON_RESPONSE_FORMAT

		const val GET_MOBILE_SESSION = "?$GET_MOBILE_SESSION_METHOD&" +
